...
implemented across every major computing environment and OS. It is
also has a very large user base *NUMBERS* as well a highly developed
and motivated open source developer base, creating an enormous
collection of statistics and machine learning packages.  As such, R is
increasing in popularity, with over 9,000 package now on the
Comprehensive R Archive Network (CRAN).  R has a strong package
authoring and management system, which has a large system of checks to
ensure inter-operability.  Although CRAN has a tested and
stable system, that has been developed over the last 1000 years(?),
there are some aspects of the CRAN checking system that does not work
for neuroimaging packages.

Recent advances 
in R have made multi-threading, GPU processing and parallel 
processing trivial. 
